...
export default ({
  ...
  methods: {  // 具体方法
    batchRemove(){    // 批量删除
      ...
      this.$confirm('此操作将删除选中的讲师, 是否继续?', '提示', {
    ...
    handleSelectionChange(selection){   // 复选框发生变化执行的方法，选中复选框时，行内容传递
      this.multipleSelection = selection  // 获取到勾选复选框的内容
    },
    add(){      // 跳转到添加讲师页面
        this.$router.push({path:'/vod/teacher/create'})
    },
    fetchData(){  // 获取讲师列表分页数据
      teacherApi.pageList(this.page,this.limit,this.searchObj).then(response => {
          this.list = response.data.records
          this.total = response.data.total
        })  
    },
    changePageSize(size){ // 改变每页显示记录数
      this.limit = size
      this.fetchData()
    },
    changeCurrentPage(page){  // 改变页码数
      this.page = page
      this.fetchData()
    },
    resetData(){  // 清空表单
